...
def test_memory_usage(
    file_size_mb: int = 100, chunk_size_mb: int = 10, memory_limit_mb: int = 100
):
    ...
    assert (
        mem_after - mem_before < memory_limit_mb
    ), f"Memory usage is {mem_after}mb, which is more than {memory_limit_mb}mb"
